...
async function main() {
  console.log('🚀 Starting update for composition nt-001...');

  const compositionId = 'nt-001';

  // 1. Find or create Raga "Anandabhairavi"
  const ragaName = 'Anandabhairavi';
  const raga = await prisma.raga.upsert({
    where: { name: ragaName },
    update: {},
    create: { name: ragaName },
  });
  console.log(`🎵 Raga "${ragaName}" resolved. ID: ${raga.id}`);

  // 2. Find or create Tala "Adi"
  const talaName = 'Adi';
  const tala = await prisma.tala.upsert({
    where: { name: talaName },
    update: {},
    create: { name: talaName },
  });
  console.log(`🥁 Tala "${talaName}" resolved. ID: ${tala.id}`);

  // 3. Define the Kannada lyrics, transliteration, and translation (English meaning)
...
Verse 7: Worship Hari, who is revered by Brahma and others. Give up lust and other evil desires. Worship Narahari (the Lord), who is praised by the virtuous, and seek the feet of Raghupati.`;
  ...
  // 4. Update the Composition in Database
  ...
  console.log('✅ Composition updated in database.');

  // 5. Update Translation in Database
  ...
  await prisma.translation.create({
    data: {
      compositionId,
      english: translation,
      kannadaMeaning: 'Updated from official source',
      wordByWord: 'Updated from official source',
    },
  });
  console.log('✅ Translation updated in database.');

  // 6. Update current_data.json file
    ...
    if (composer.compositions) {
      for (const comp of composer.compositions) {
        if (comp.id === compositionId) {
          comp.title = 'Entu Marulade Naa Nentu Marulade';
          comp.firstLine = 'Entu marulade naa nentu marulade';
          comp.lyrics = kannadaLyrics;
          comp.transliteration = transliteration;
          comp.ragaId = raga.id;
          comp.talaId = tala.id;
          updatedCount++;
        }
  ...
  if (updatedCount > 0) {
    ...
    console.log(`✅ Updated ${updatedCount} occurrences of nt-001 in current_data.json`);
  } else {
    console.log('❌ Could not find nt-001 in current_data.json to update.');
  }

  console.log('🎉 Database and JSON sync completed successfully!');
}
