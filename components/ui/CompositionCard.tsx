...
export default function CompositionCard({
  ...
  const getKannadaLines = () => {
    ...
    return lyrics.split('\n')
      .map(l => l.trim())
      .filter(l => l && !l.toLowerCase().includes('pallavi') && !l.includes('ಪಲ್ಲವಿ'));
  };
    ...
    >
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-100 mt-auto">
            <button 
                ...
                className={`p-3 min-h-[44px] min-w-[44px] flex items-center justify-center transition-colors ${saved ? 'text-primary' : 'text-slate-400 hover:text-primary'}`}
                aria-label={saved ? "Remove bookmark" : "Add bookmark"}
            >
                {saved ? <BookmarkCheck className="h-5 w-5 fill-current"/> : <Bookmark className="h-5 w-5"/>}
            </button>
            <Link href={`/library/${id}`} className="flex items-center gap-1.5 text-xs font-bold text-primary hover:text-primary/80 transition-colors p-3 min-h-[44px]">
